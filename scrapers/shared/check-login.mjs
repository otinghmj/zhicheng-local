...
/**
 * 查询登录状态——不弹窗、不阻塞、不 exit，只返回结果。
 * 供"非阻塞提示"（如猎聘匿名可搜）和"结果异常时回查真失效"使用。
 *
 * @param {string} platform
 * @param {object} opts
 *   @param {string}  opts.cdpUrl
 *   @param {boolean} opts.verify  true=额外做浏览器内 API 验证（能识别 cookie 还在但服务端已失效）
 * @returns {Promise<{ supported:boolean, loggedIn:boolean|null, reason:string }>}
 */
export async function getLoginStatus(platform, { cdpUrl = "http://127.0.0.1:9223", verify = false } = {}) {
  const config = PLATFORM_CONFIG[platform];
  if (!config) return { supported: false, loggedIn: null, reason: "unsupported" };

  const cookies  = await getDomainCookies(cdpUrl, config.domain).catch(() => []);
  const cookieOk = hasValidAuthCookie(cookies, config.authCookies);
  if (!cookieOk) return { supported: true, loggedIn: false, reason: "no-cookie" };
  if (!verify)   return { supported: true, loggedIn: true,  reason: "cookie-ok" };

  let v = "ok";
  try { v = await verifyLoginInBrowser(cdpUrl, config); } catch { v = "ok"; } // 验证异常时乐观视为已登录，避免误判
  return v === "ok"
    ? { supported: true, loggedIn: true,  reason: "verified" }
    : { supported: true, loggedIn: false, reason: "server-expired" };
}
