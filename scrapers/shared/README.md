...
- `scrapers/boss/boss-cdp.mjs` — 自动启动后执行 BOSS 采集（CDP-Scroll 模式，joblist.json 网络监听，无 Hammerspoon）
- `scrapers/zhaopin/zhaopin-cdp.mjs` — 自动启动后执行智联采集（CDP-JS-State 模式，无 Hammerspoon）
