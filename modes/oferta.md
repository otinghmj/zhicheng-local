...
基于 Bloque A 检测到的岗位类型，映射到对应子目录，并创建该目录（如不存在）：

| 岗位类型关键词 | 子目录 |
|--------------|-------|
| 工业AI / AI应用工程师 / LLM工程师 / AI工程师 / 工业数字化工程师 / 制造AI | `reports/主线A-工业AI/` |
| 工业软件PM / SaaS产品经理 / 工业互联网PM / 工业互联网产品经理 / 制造SaaS | `reports/主线B-工业软件PM/` |
| 技术项目经理 / PMO / 研发项目经理 / 数字化项目经理 | `reports/备选-技术PM/` |
| RAG工程师 / 知识图谱工程师 / 向量检索 / NLP工程师 | `reports/探索-RAG/` |
| 无法明确归类 / 解决方案架构师 / 其他 | `reports/` 根目录 |

### 2. Guardar report .md

Guardar evaluación completa en `reports/{子目录}/{###}-{company-slug}-{YYYY-MM-DD}.md`。

- `{子目录}` = 上一步确定的子目录（根目录则不加前缀）
- `{###}` = siguiente número secuencial (3 dígitos, zero-padded)，全局统一编号，不按子目录单独计数
...
- Report: link relativo al report .md，含子目录（ej: `[011](reports/主线A-工业AI/011-zpmc-2026-05-15.md)`）
