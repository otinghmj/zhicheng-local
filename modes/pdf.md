...
13. Escribe HTML a `output/.html-cache/cv-{company}-{YYYY-MM-DD}.html`
14. Ejecuta: `node generate-pdf.mjs output/.html-cache/cv-{company}-{YYYY-MM-DD}.html output/cv/cv-candidate-{company}-{YYYY-MM-DD}.pdf --format={letter|a4}`
15. Elimina el HTML temporal: `rm output/.html-cache/cv-{company}-{YYYY-MM-DD}.html`
...
b. **IMMEDIATELY** download the PDF using Bash:
   ```bash
   curl -sL -o "output/cv/cv-{candidate}-{company}-canva-{YYYY-MM-DD}.pdf" "{download_url}"
   ```
   The export URL is a pre-signed S3 link that expires in ~2 hours. Download it right away.
c. Verify the download:
   ```bash
   file output/cv/cv-{candidate}-{company}-canva-{YYYY-MM-DD}.pdf
   ```
